...
const expr=String.raw`(() => {
  ...
  const userLike = hooks.filter((h)=>h.value && typeof h.value==='object' && !Array.isArray(h.value) && (('uid' in h.value)||('authUid' in h.value)||('phone' in h.value)||('storeId' in h.value)||('isAuthenticated' in h.value)||('phoneNumber' in h.value))).map((h)=>({idx:h.idx,value:sanitize(h.value)}));
  const storageKeys = Object.keys(localStorage).filter((key)=>key.includes('liwu_'));
  return {
    ok:true,
