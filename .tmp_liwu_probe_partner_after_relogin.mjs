...
const expr=String.raw`(() => {
  ...
  while(hook && idx<160){ hooks.push({ idx, value:getHookValue(hook) }); hook=hook.next; idx+=1; }
  ...
  return {
    ...
    userLike: objHooks.filter((h)=>('uid' in h.value)||('authUid' in h.value)||('phone' in h.value)||('storeId' in h.value)||('storeRole' in h.value)).map((h)=>({idx:h.idx,value:sanitize(h.value)})).slice(0,20),
    authLike: objHooks.filter((h)=>('isAuthenticated' in h.value)||('phoneNumber' in h.value)||('isAnonymous' in h.value)).map((h)=>({idx:h.idx,value:sanitize(h.value)})).slice(0,20),
