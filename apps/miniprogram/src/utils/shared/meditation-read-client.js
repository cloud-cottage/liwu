...
const MEDITATION_READ_FUNCTION_NAME = 'meditation-read'

// 临时链接有效期（秒）：与规范 C11 / 质检 X14 的 `maxAge = 7200` 一致。
const MEDITATION_READ_URL_MAX_AGE_SECONDS = 7200

// 端侧缓存陈旧阈值（秒）：R39 ⑪「缓存 ≤ max_age_seconds 的一半（≈1 小时）」。
const MEDITATION_READ_URL_STALE_AFTER_SECONDS = MEDITATION_READ_URL_MAX_AGE_SECONDS / 2
...
const resolveMeditationUrlPolicyStaleness = ({
  urlPolicy = null,
  receivedAtMs = null,
  nowMs = Date.now()
} = {}) => {
...
const MEDITATION_READ_ACTIONS = Object.freeze({
  getTrack: 'getTrack',
...
// 缺省 action＝端侧主路径（R39 ②）。
const MEDITATION_READ_DEFAULT_ACTION = MEDITATION_READ_ACTIONS.getTrack

// 六章固定模板的章数（R10 / R39 ⑦：Track 一律折回六章模板，`chapters` 恒 6）。
const MEDITATION_READ_REQUIRED_CHAPTER_COUNT = 6

const MEDITATION_READ_ERROR_CODES = Object.freeze({
  // 服务端下发的 error 码（R39 ③）
...
class MeditationReadError extends Error {
...
const isMeditationReadError = (error) => (
...
const validateMeditationReadPayload = ({ action = MEDITATION_READ_DEFAULT_ACTION, data = null } = {}) => {
  const issues = []
...
const assertMeditationReadPayload = ({ action, data } = {}) => {
...
const createMeditationReadClient = ({ callFunction, thisArg = undefined } = {}) => {
...
module.exports = {
  ...
  MEDITATION_READ_URL_MAX_AGE_SECONDS,
  MEDITATION_READ_URL_STALE_AFTER_SECONDS,
  resolveMeditationUrlPolicyStaleness,
  MEDITATION_READ_ACTIONS,
  MEDITATION_READ_DEFAULT_ACTION,
  MEDITATION_READ_REQUIRED_CHAPTER_COUNT,
  MEDITATION_READ_ERROR_CODES,
  MeditationReadError,
  isMeditationReadError,
  validateMeditationReadPayload,
  assertMeditationReadPayload,
