...
const expr=String.raw`(() => {
  ...
  const arr=(idx)=>Array.isArray(valueAt(idx)) ? valueAt(idx) : [];
  const userLike = hooks.filter((h)=>h.value && typeof h.value==='object' && !Array.isArray(h.value) && (('uid' in h.value)||('authUid' in h.value)||('phone' in h.value)||('storeId' in h.value)||('storeRole' in h.value)||('name' in h.value))).map((h)=>({idx:h.idx,value:sanitize(h.value)}));
  return {
    ok:true,
    scalarStrings: hooks.filter((h)=>typeof h.value==='string').map((h)=>({idx:h.idx,value:h.value})).slice(0,60),
    livePartnerUsersLen: arr(18).length || arr(16).length,
    livePartnerBrandsLen: arr(19).length || arr(17).length,
    livePartnerMembersLen: arr(20).length || arr(18).length,
    partnerUsersWithPhone166: sanitize((arr(18).length ? arr(18) : arr(16)).filter((user)=>String(user?.phone||'')==='16601061656').slice(0,5)),
    partnerBrandsStore102: sanitize((arr(19).length ? arr(19) : arr(17)).filter((brand)=>String(brand?.slug||'')==='store_102').slice(0,5)),
    userLike,
