...
const expr=String.raw`(() => {
  ...
  const seen = new Set();
  const stack = [start];
  let profileFiber = null;
  while (stack.length > 0) {
    const current = stack.pop();
    if (!current || seen.has(current)) continue;
    seen.add(current);
    const name = current.elementType?.name || current.type?.name || '';
    if (name === 'Profile') { profileFiber = current; break; }
    if (current.child) stack.push(current.child);
    if (current.sibling) stack.push(current.sibling);
  }
  if (!profileFiber) return { ok:false, error:'profile_fiber_not_found' };
  ...
  const hooks=[]; let hook=profileFiber.memoizedState; let idx=0;
  while(hook && idx<120){ hooks.push({ idx, value:getHookValue(hook) }); hook=hook.next; idx+=1; }
  const objHooks = hooks.filter((h)=>h.value && typeof h.value==='object' && !Array.isArray(h.value));
  const userLike = objHooks.filter((h)=>('uid' in h.value)||('authUid' in h.value)||('phone' in h.value)||('name' in h.value)||('displayName' in h.value)||('isAuthenticated' in h.value)).map((h)=>({idx:h.idx,value:sanitize(h.value)}));
  return {
    ok:true,
    title: document.title,
    scalarStrings: hooks.filter((h)=>typeof h.value==='string').map((h)=>({idx:h.idx,value:h.value})).slice(0,40),
    scalarBooleans: hooks.filter((h)=>typeof h.value==='boolean').map((h)=>({idx:h.idx,value:h.value})).slice(0,20),
    userLike,
