...
const expr=String.raw`(() => {
  const sanitize = (value) => JSON.parse(JSON.stringify(value, (key, inner) => {
  ...
  while(hook && idx<140){ hooks.push({ idx, value:getHookValue(hook) }); hook=hook.next; idx+=1; }
  const valueAt=(idx)=>hooks.find((h)=>h.idx===idx)?.value;
  const arrAt=(idx)=>Array.isArray(valueAt(idx)) ? valueAt(idx) : [];
  const resolvedCandidates = hooks.filter((h)=>h.value && typeof h.value==='object' && !Array.isArray(h.value) && (('uid' in h.value)||('storeId' in h.value)||('storeOwnerUserId' in h.value)||('authUid' in h.value))).map((h)=>({idx:h.idx,value:sanitize(h.value)}));
  return {
    ok:true,
    verifiedPhone:valueAt(3),
    activeRole:valueAt(0),
    activeModule:valueAt(1),
    scalarStrings: hooks.filter((h)=>typeof h.value==='string').map((h)=>({idx:h.idx,value:h.value})).slice(0,30),
    liveShopProductsLen: arrAt(11).length,
    livePartnerUsersLen: arrAt(16).length,
    livePartnerBrandsLen: arrAt(17).length,
    livePartnerBrandMembersLen: arrAt(18).length,
    resolvedCandidates,
    store102Products: sanitize(arrAt(11).filter((item)=>String(item?.storeId||'')==='store_102').slice(0,3))
  };
})();`;
