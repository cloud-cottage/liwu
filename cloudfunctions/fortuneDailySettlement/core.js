const { resolveProductTypeByCategoryName } = require('./product-type.js');

const DAILY_BEANS_BASE_AMOUNT = 100;
const CONFIRMED_STATUSES = new Set(['shipped', 'completed', '已发货', '已完成']);
...
const calculateDailyBeansConsumption = (
...
const isPhysicalProduct = (categoryName = '', productType = '') => {
  if (productType) {
...
const runFortuneDailySettlement = async ({
  collections,
  queryAll,
  updateBrand,
  updateUser,
  ...
  for (const brand of partnerBrands) {
    ...
    const dailyConsumption = calculateDailyBeansConsumption(shopYesterdaySales, platformYesterdayTotalSales, last7DaysOrderCount, last30DaysOrderCount);
